...
policy_net = DQN(board.shape[0], board.shape[1], n_actions).to(device)
target_net = DQN(board.shape[0], board.shape[1], n_actions).to(device)
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        result = env.do_turn(action.item()) # env.step(action.item())
        done = (result != -1) # invalid move

        if result == 1:
            reward = 100 # FIRST PLAYER WINS
        if result == 2:
            reward = -100 # SECOND PLAYER WINS
        if result == -2:
            reward = -1  # TIE
        if result == 0:
            reward = 0 
